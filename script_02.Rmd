...
```{r}
raw <- read_xlsx("~/gov50/class-2020-09-17/raw_data/class_enrollment_summary_by_term_9-17-2020.xlsx", skip = 3)

en_fall_2020 <- raw %>%  
  clean_names() %>%
  drop_na() %>%
  filter(u_grad >= 10) %>%
  select(-c(grad, non_degree, x_reg, vus, employee))

highest_enrollment <- en_fall_2020 %>%  
  arrange(desc(u_grad)) %>%
  slice(1:1)
# the class with the highest enrollment is...
highest_enrollment
...
en_fall_2020 %>%
  arrange(desc(u_grad)) %>%
  slice(1:9) %>%
  ggplot(aes(x = fct_reorder(course_title, u_grad), y = u_grad)) + geom_col() + coord_flip()
```
### Scene 3
